...
printf("%c",65+i); for(j=0;j<n;j++) scanf("%d",&edge[i][j]);
...
p=find(u); q=find(v); if(p!=q)

{ t[i][0]=u;

t[i][1]=v; mincost=mincost+edge[u][v]; sunion(p,q);
...
if(parent[I]>0) I=parent[I]; return I;
